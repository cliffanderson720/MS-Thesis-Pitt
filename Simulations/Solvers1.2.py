...
amin = 1 # wave speed
tmin, tmax = 0.0, 2.5 # start and stop time of simulation
...
dt = c/amin*dx # stable time step calculated from stability requirement
...
for k, solver in enumerate(solvers): # Solve for all solvers in list
    ...
    for i, t in enumerate(time[1:]):
        ...
        if k==0:
            uanalytical[i,:] = f(x-amin*t) # compute analytical solution for this time step
            
        u_bc = interpolate.interp1d(x[-2:], u[-2:]) # interplate at right bndry
        
        u[1:-1] = solver(u[:]) # calculate numerical solution of interior
        u[-1] = u_bc(x[-1] - amin*dt) # interpolate along a characteristic to find the boundary value
