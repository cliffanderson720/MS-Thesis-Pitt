...
PEOsum <- PEOmeoh[PEOmeoh$Cycle<8,] %>% group_by(WaferID,Cycle) %>% summarise(n=n(),mean = mean(Angle),sd = sd(Angle),se=sd/sqrt(n))
...
ggplot(PEOsum,aes(Cycle,mean,color=WaferID))+
  ...
  ylab('Inclination angle')+ggsave('PEO_coating_summary.png')
...
with(clean,{
  pairwise.t.test(Angle,Depth,alternative='less')})
